...
import com.dhk.kernel.util.XmlUtil;
...
public class DreamConf {
	...
	protected static Map<String,ServiceDef> serviceDefs = new HashMap<String,ServiceDef>();
	
	protected static List<ServerSupport> serverList = new ArrayList<ServerSupport>();
	
	protected static DeployDef currentDeploy;	
	 ...
	 /**
	  ...
	  public static void loadArtConf() {
	    try {
	      ...
	      e.printStackTrace();
	      throw new BaseException("fail to to load /conf/appConf.xml", e);
	    }
	  }
	  ...
	  public static void parseServices(Element root) {
		    try {
		      ...
		      for (int i = 0; i < serviceNodes.getLength(); i++) {
		        ...
		        ServiceDef sd = (ServiceDef) serviceDefs.get(sTemp);
		        if (sd == null) {
		          sd = new ServiceDef();
		          sd.setName(sTemp);
...
//		        server.joinCluster = !"false".equals(XmlUtil.getAttribute(serverNode,"joinCluster"));
	  ...
	  private static void parseProperties(Element root) {
		    ...
		    for (int i = 0; i < propNodes.getLength(); i++) {
		      ...
		      if (name == null || value == null)
		        throw new BaseException("<property>的name和value属性不能为空");
		      if (DEFAULT_PASSWORD.equals(name)){
		    	  throw new BaseException("<property>的name不能为系统默认的["+DEFAULT_PASSWORD+"]");
		      }
		      properties.put(name, value);
		    }
	  ...
	  public static void parseDeploy(Element root) {
		    try{
		      ...
		      if(deployNodes.getLength() != 1){
		    	...
		    	e.printStackTrace();
		        throw new BaseException("fail to to load conf/appInfo.xml", e);
		    }
	...
	public static Map<String,ServiceDef> getServiceDefs() {
	    return serviceDefs;
	}
	public static DeployDef getCurrentDeploy(){
