...
public abstract class JdbcBaseDaoSupport <T extends Entity, PK extends Serializable> implements IBaseDao<T, PK>{
    ...
    public long insert(T t){
    	...
    	long m=keyholder.getKey().longValue();
   ...
   public long insert(String sql,T t){
	   KeyHolder keyholder=new GeneratedKeyHolder();
	   SqlParameterSource ps=new BeanPropertySqlParameterSource(t);
    ...
    public com.sunnada.kernel.dao.Pager<T> findForPage(String sql, Map<String,Object> map, com.sunnada.kernel.dao.Pager<T> pager){
    	...
    	int e = psql.indexOf("order");
    	if (e>0){
    		psql = psql.substring(0,e);
    	}
...
//    	return getNameParameterJdbcTemplate().query(sql, map, new BeanPropertyRowMapper(entityClass));
    ...
    public com.sunnada.kernel.dao.Pager<T> queryForPage(String sql, Map<String,Object> map, com.sunnada.kernel.dao.Pager<T> pager){
    	int index = sql.indexOf("from");
    	String psql = sql.substring(index,sql.length());
    	int e = psql.indexOf("order");
    	if (e>0){
    ...
    public com.sunnada.kernel.dao.Pager<T> findForPage2OrderBy(String sql, Map<String,Object> map, com.sunnada.kernel.dao.Pager<T> pager, String orderBySql){
    	instanEntity();
    	int index = sql.indexOf("from");
    	String psql = sql.substring(index,sql.length());
    	//index = psql
