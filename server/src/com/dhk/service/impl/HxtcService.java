...
import com.dhk.utils.RsaDataEncryptUtilDh;
 ...
 */
@Service("hxtcService")
public class HxtcService {
	static Logger logger =LogManager.getLogger(HxtcService.class);
	...
	public JSONObject findWithdraw(String orderDate,String orderNo) throws Exception {
		...
		try{
			/**设置公用参数并签名*/
			RRParams requestData = RRParams.newBuilder().setAppId(Constant.appIddh).setClientTransId(generateTransId())
					.setTransTimestamp(trans_timestamp).setTransType(trans_type).build();
			logger.info("提交的报文为:"+businessReq);
			/**请求接口*/
			ResponseDataWrapper rdw = HttpUtils.post(Constant.requestUrl, requestData, businessReq,
