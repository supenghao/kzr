package com.sunnada.kernel.sql;
...
public class SQLConf {
	public static com.sunnada.kernel.AppContext appContext = new com.sunnada.kernel.DefaultAppContext();
	...
	public static SQLDef getSqlDef(String sqlFileName,String sqlName){
		if(sqlDefs.get(sqlFileName)==null){
			try {
				loadSql();
			} catch (Exception e) {
				e.printStackTrace();
	...
	private static Map<String,SQLDef> parseXml(File f) throws Exception{
		Map<String,SQLDef> defs = new HashMap<String,SQLDef>();
	      ...
	      if (is == null)
				throw new com.sunnada.kernel.exception.BaseException("未找到XML文件对应的输入流...");
